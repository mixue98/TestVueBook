<template>
  <div class="bg">
    ...
    <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm register-container">
      ...
      <router-link to='../admin' class="link-re">已有账号，去登录！</router-link>
    </el-form>
  </div>
</template>
<script>
  import {managerRegister} from '../../api/account'
  export default {
    data () {
    ...
    methods: {
      submitForm (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            let data = {
              mname: self.ruleForm2.account,
              mpwd: self.ruleForm2.checkPass
            }
            managerRegister(data)
              .then(function(res){
              	debugger
                if(res.data==="注册成功,请登陆"){
                  self.$message({message:"注册成功，请登陆",duration:1500})
                  window.location.href = '../admin'
                }else if(res.data==="用户名存在！"){
                  self.$message({message:"用户名存在！",duration:1500})
                }else {
                  self.$message({message:"注册失败！",duration:1500})
                }
              })
              .catch(function(err){
                self.$message({message:"异常！",duration:1500})
              })
          } else {
            console.log('no')
          return false
